...
description: Use this skill to research customer-specific questions and investigate account contexts. Search across internal documentation, knowledge bases, CRM records, and connected sources to answer questions about specific customer accounts, their history, product usage, support tickets, or inquiries. Synthesize findings with confidence scores. Trigger when users need to investigate a particular customer's situation, verify information for a specific account, or gather background on customer interactions. Do not use for general competitive analysis, skill evaluation, or tasks unrelated to specific customer accounts.
---
...
**When writing about specific customers — especially KDM names, titles, org structure, or personnel — NEVER guess, infer, or fabricate. Not even as a placeholder.**

This includes:
- Making up names because analysis "needs" a person in a role (e.g., 张盛泽 was fabricated)
- Attributing real people to the wrong company (e.g., 陈波/王冰冰 are 智能派, not 拓竹)
- Assuming org structure without evidence (e.g., "four KDMs each managing a silo")
- Using placeholder names and forgetting to replace them

**The rule**: If you need customer personnel data, pull it from C360 CLI first:
```bash
lark-c360 follow_up +recent --account-id <entity_id> --field contacts --field contact_titles --limit 30 --json
```
This extracts actual KDM names and roles from real engagement records. If C360 has no data for the account, say "C360 has no KDM data" rather than inventing.

**Why this matters**: Fabricated customer data erodes trust and can cause real business mistakes.

See `c360-cli` skill for the full contacts extraction workflow.

## Multi-Source Research Methodology

### Research Process

**Step 0: Pull C360 ground truth FIRST**
Before any synthesis about a specific customer, pull the facts from C360. For KDM/contacts, use the `follow_up +recent` contacts extraction pattern from `c360-cli`. Never write about customer personnel without verifying against C360.
