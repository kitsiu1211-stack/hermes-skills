...
交付前把「红线」清单逐条过一遍（这是硬门禁，不是打分）：

- 口语检验：读出声来，你会这样跟一个聪明的朋友说话吗？
- 零术语：12 岁孩子能复述吗？
- 短词优先 / 一句一事 / 具体 / 开头给理由 / 不填充 / 信任读者 / 诚实

准确度检查：对着原文逐段核对，有没有曲解或遗漏？改写的每件事都能在原文章里找到出处？
...
|--------|----------|
| 常规/内部产物 | 红线自查 |
| 对外/高价值产物（发客户、发公众号） | 红线自查 + **外派独立 Evaluator** |

外派规则：delegate_task 起独立子 agent，异构优先（flash 产出 → pro 审查或换模型家族），输出**具体修改点清单**而不是评分，只修真问题。
