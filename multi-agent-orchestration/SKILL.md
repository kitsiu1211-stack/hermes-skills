...
本 Skill 编排多 Agent 产出合并结果，**合并后必须经过 Evaluator 独立评分后才能交付**。

| 维度 | 阈值 | 检查要点 |
|------|------|---------|
| **子任务完整性** | ≥7 | 所有子 Agent 的结果是否都已收到？有无遗漏？ |
| **合并一致性** | ≥7 | 不同 Agent 的结果之间有无矛盾？矛盾是否被标注而非掩盖？ |
| **结论清晰度** | ≥7 | 合并后的最终输出是否有明确的结论/行动建议？还是「信息堆砌」？ |
