---
name: ljg-rank
description: 给一个领域，找出背后真正撑着它的几根独立的力。十几个现象砍到不可再少的生成器——砍完能把现象一个个生回来，才算数。Use when user says '降秩', '找秩', '秩是什么', '这个领域靠什么撑着', '背后是什么', '深度拆解', or wants to decompose any domain to its irreducible generators. Also triggers when user shares a WeChat/long-form article for analysis.
user_invocable: true
---
...
当作为公众号文章分析流水线的一部分运行时，ljg-rank 与 ljg-learn 配对使用：rank 先找骨架，learn 随后从中挑一个核心概念解剖。完整流水线见 `feishu-api` skill 的 `references/article-analysis-pipeline.md`。
...
2. 写入 `~/Documents/Hermes Context/articles/{时间戳}--{领域}的秩__rank.md`
3. **生成飞书卡片**（必须）：用 Python subprocess + feishu-cli 发送交互式卡片，摘要核心发现：
   - Header：`{领域}的秩`，template 用 blue
   - 第一段：root rank（两根生成器是什么，各管什么）
   - 第二段：坐标系（如果有叠），四象限各对应什么
   - Note：数据来源/关键数据点
   
   卡片内容精炼——只放骨架，不放全文。完整散文留给 Obsidian。卡片 JSON 通过 `execute_code` 调用 `feishu-cli exec im.v1.message.create` 发送，参考 `feishu-api` skill 的「Shell escaping trap」专用模式。`receive_id` 用 `ou_dc055b0b5b0b5db2b1af5e79c0536db6`，`receive_id_type` 用 `open_id`。
4. 告知用户卡片已推送 + Obsidian 文件路径

## 质检：Generator → Evaluator（强制）

本 Skill 产出用户可见内容（分析报告 + 飞书卡片），**必须经过 Evaluator 独立评分后才能交付**。

### 身份切换

- **Generator（你—创建者）**：执行上述 1-4 步，生成完整输出。完成后**切换身份**。
- **Evaluator（你—审查者）**：以全新视角审视 Generator 的输出。你不是生成者，你是质检员。你的唯一目标是挑毛病。

### 三维度评分

| 维度 | 满分 | 阈值 | 检查要点 |
|------|------|------|---------|
| **生成力完整性** | 10 | ≥7 | 用找到的生成器能否逐一生回用户提到的现象？少一个都不合格。 |
| **独立性验证** | 10 | ≥7 | 生成器之间是否真的独立？有没有一根可以被另一根推导出来？ |
| **中文表达质量** | 10 | ≥7 | 口语化、中文母语感、无翻译腔、无 AI 八股句式。 |
...
Evaluator 独立评分（三维度）
       ↓
  ┌─ 全部 ≥7 → 交付卡片 + Obsidian
  └─ 任一 <7 → 标注不合格维度 → Generator 修正 → Evaluator 重评（最多 3 轮）
                                                        ↓
                                              3轮仍不合格 → 告知用户+说明原因
```
