...
## Before generating any page: Load Design Tokens FIRST

**This is a hard constraint, not a suggestion.** Before writing a single line of HTML/CSS/JSX for a UI, read `references/apple-tokens.css`. This file contains Apple's real HIG values translated into CSS variables:

- **Color system**: backgrounds, labels, system tints, fills, separators (light + dark mode)
- **Spacing scale**: 4 / 8 / 12 / 16 / 20 / 24 / 32 / 40 px
- **Border radius**: 6 / 8 / 13 / 20 / 26 px (matching iOS continuous corners)
- **Typography**: font sizes, line heights, weights, letter-spacing (iOS Dynamic Type scale)
- **Shadows**: 4-tier elevation system
- **Materials**: blur/translucency values
- **Animation**: spring curves, durations, opacity states
- **Accessibility**: prefers-reduced-motion, prefers-contrast

**All visual properties in generated code MUST reference these CSS variables.** Hardcoded color hex codes, px spacing, or font values that don't match the token set are rejected. When you need a spacing value, pick from `--apple-spacing-*` — never write `padding: 15px` when the scale offers 12px and 16px.

This is the "Hook" layer: the token file is the source of truth that the LLM cannot override. The LLM chooses which token to use, but it cannot invent new values.

Full methodology and anti-patterns: see `references/hook-enforcement.md`.

### Neat-Freak Convergence: One truth, one place

The "洁癖" principle applied to design systems: **every visual fact must have exactly ONE authoritative source.** If `--apple-radius-lg: 13px` is defined in the token file, no component file should ever contain `border-radius: 13px` — it must reference `var(--apple-radius-lg)`. Duplicating the value in two places means they will eventually diverge.

Before committing any generated UI, run this self-audit:

1. Scan output for any hardcoded hex colors → must be `var(--apple-*)` or rejected
2. Scan for any magic-number spacing → must come from the `--apple-spacing-*` scale
3. Scan for duplicated values across files → consolidate to one token reference

The convergence check is the "Verify Hook" — it catches what the LLM might have missed.
